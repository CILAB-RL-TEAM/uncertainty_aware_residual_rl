...
flags.DEFINE_string("robomimic_checkpoint_path", None, "Path to robomimic checkpoints.")

flags.DEFINE_string("demo_path", None, "Path to demonstration data")
...
flags.DEFINE_string("diffusion_config_dir_path", None, "Directory for Config files for diffusion policy")
...
def train_loop(agent: SACAgent, replay_buffer, env, eval_env, sampling_rng, wandb_logger):
    ...
    The residual policy is only applied when the ensemble uncertainty exceeds a threshold.
    ...
    #Load base policies
    diffusion_policies=[]
    for dirpath, dirnames, filenames in os.walk(FLAGS.diffusion_config_dir_path):
        for filename in filenames:
            file_path = os.path.join(dirpath, filename)
            print_green(f"Loading diffusion policy from {file_path}")
            cfg = OmegaConf.load(file_path)
            diffusion_policy = hydra.utils.instantiate(cfg.model)
            diffusion_policies.append(diffusion_policy)
    ...
    def get_base_action_and_var(obs):
        """Get action from diffusion policy ensemble and compute action variance.
        
        Generates actions from all policies in the ensemble and computes the standard
        deviation across the ensemble to measure uncertainty.
        
        Args:
            obs: Current observation from the environment.
            
        Returns:
            Tuple of (action, variance) where action is from the first policy in
            the ensemble and variance is the mean standard deviation across action dimensions.
        """
        ## Get action from the diffusion policy and the ensemble variance for that action
        state = normalize_obs(obs)
        cond = {"state": torch.from_numpy(state).float().to(device)}
        cond["state"] = torch.unsqueeze(cond["state"], 0)
        cond["state"] = torch.unsqueeze(cond["state"], 0)
        actions = []
        for diffusion_policy in diffusion_policies:
        ...
        ## Choose action from one policy 
        ## To maximize performance, you can choose the actions from the best performing policy of the ensemble
        act = actions[0]
        var = np.std(actions, axis=0)
        return act, np.mean(var)
    ...
    for step in tqdm.tqdm(range(FLAGS.max_steps), dynamic_ncols=True):
        ...
        with timer.context("sample_actions"):
            #get base action and ensemble variance  
        ...
        with timer.context("step_env"):
            ...
            next_base_action,_ = get_base_action_and_var(next_obs)
        ...
        with timer.context("learner"):
            #if step > FLAGS.random_steps and step%FLAGS.learner_update==0:
        ...
        def residual_action(obs):
            """Compute final action using residual policy based on uncertainty.
            
            If ensemble uncertainty is below threshold, uses base policy action only.
...
def main(_):
    ...
    if FLAGS.env == "MujocoState":
        ...
        if FLAGS.robomimic_checkpoint_path is not None :
        ...
        rm_env.env.reward_shaping = (
            False  #Reward shaping should be true
        )
        rm_env.env.horizon = (
    ...
    #Zero out the last layer of the acto network for residual policy, check which layer to zero out, check after burn in if stil 0 (right after)
