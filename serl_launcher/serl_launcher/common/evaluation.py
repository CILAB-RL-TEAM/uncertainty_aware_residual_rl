...
def evaluate(policy_fn, env: gym.Env, num_episodes: int, base_policy: None) -> Dict[str, float]:
    ...
    for _ in range(num_episodes):
        ...
        while not done:
            action, action_used = policy_fn(observation)
            base_actions_used = base_actions_used + action_used
    ...
    stats["base_actions_used"] = base_actions_used/total_actions
    return stats

def evaluate_with_stats(policy_fn, env, num_episodes: int, base_policy: None, normalize_reward=1) -> Dict[str, float]:
    stats = defaultdict(list)
    base_actions_used = 0
    total_actions = 0
    success = 0
    ...
    stats["base_actions_used"] = base_actions_used/total_actions
    stats["episode.s"] = success/num_episodes
    stats["episode.r"] = total_reward/num_episodes
    stats["episode.normalized.r"] = total_reward/(num_episodes*normalize_reward)
    stats["episode.l"] = total_actions/num_episodes
    return stats
